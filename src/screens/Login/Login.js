...
const Login = () => {
	...
	const loginAccountApi = useMutation(LoginAccount, {
		onSuccess: (res, formParams) => {
			if (res?.status) {
				dispatcher({type: USER_LOGIN, payload: {user: res?.data?.user_info, access_token: res?.data?.auth_token}})
			}
		},
	})
	...
	const forgottenPasswordApi = useMutation(ForgottenPassword, {
		onSuccess: (res, formParams) => {
			if (res?.status) {
				...
				dispatcher({type: FLUSDYNAMIC_STORE, payload: {store: "resetPwdSession", data: {user: formParams?.partner_email, bottomSheetView: 2}}})
			}
		},
	})
	...
	const verifyPasswordResetApi = useMutation(VerifyPasswordReset, {
		onSuccess: res => {
			if (res?.status) {
				dispatcher({type: FLUSDYNAMIC_STORE, payload: {store: "resetPwdSession", data: {bottomSheetView: 3, ...res?.data}}})
			}
		},
	})
	...
	const resetPasswordApi = useMutation(ResetPassword, {
		onSuccess: res => {
			if (res?.status) {
				dispatcher({type: FLUSDYNAMIC_STORE, payload: {store: "resetPwdSession", data: {bottomSheetView: 4}}})
	...
	const destoryPasswordResetSession = () => {
		dispatcher({type: FLUSDYNAMIC_STORE, payload: {store: "resetPwdSession", data: {bottomSheetView: 0}}})
		bottomSheetRef.current?.snapToIndex(0)
	}
		...
		<>
			<KeyboardAwareScrollView contentContainerStyle={styles.container}>
				...
				<View style={styles.wrapper}>
					...
					<Formik validationSchema={accountLoginValidationSchema} initialValues={{pa_email: "", password: ""}} onSubmit={handleAccountLogin}>
					...
					<View style={styles.resendOtpView}>
						...
						<Pressable
							onPress={() => {
								...
								dispatcher({type: FLUSDYNAMIC_STORE, payload: {store: "resetPwdSession", data: {bottomSheetView: 1}}})
							}}
						>
				...
				<BottomSheet backdropComponent={renderBackdrop} backgroundStyle={styles.bottomSheet} ref={bottomSheetRef} index={-1} enablePanDownToClose snapPoints={snapPoints}>
					{/* Forgotten Password */}
					{resetPwdSession?.bottomSheetView === 1 && (
						<>
							<Formik validationSchema={forgetPwdValidationSchema} initialValues={{partner_email: ""}} onSubmit={handleAccountRecovery}>
								{({handleChange, handleSubmit, errors, touched, values, isSubmitting}) => (
									<>
										<View style={styles.contentContainer}>
											<Text style={styles.forgotText}>Forgot password</Text>
											<Text style={{...FONTS.body4}}>To reset your password enter your registered email below, we will send you a 4 digits code to your email for verification.</Text>
											<View style={{marginVertical: SIZES.font1}}>
												<InputBox
													...
													error={touched.partner_email && errors.partner_email ? errors.partner_email : null}
												/>
											</View>
											<CustomButton title="Send" style={{marginTop: SIZES.font10}} onPress={handleSubmit} isLoading={isLoading} disabled={isLoading} />
										</View>
									</>
								)}
							</Formik>
										...
										<View style={styles.contentContainer}>
											...
											<View style={{marginVertical: SIZES.font1}}>
												...
												<InputBox
													...
													error={touched.password_confirmation && errors.password_confirmation ? errors.password_confirmation : null}
												/>
											</View>
					...
					{resetPwdSession?.bottomSheetView === 4 && (
						<>
							<View style={styles.verifyView}>
								...
								<CustomButton title="Login" style={styles.startButton} onPress={destoryPasswordResetSession} />
							</View>
						</>
					)}
				</BottomSheet>
