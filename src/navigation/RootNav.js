...
import {ACCOUNT_SCREEN, ACCOUNT_TYPE, BOTTOMTAB_SCREEN, LOGIN_SCREEN, ONBOARD_SCREEN, SIGNUP_SCREEN, TERMS_AND_POLICY_SCREEN, VERIFY_OTP_SCREEN} from '../constants/screens'
			...
			<Stack.Navigator
				...
				{!auth?.isActive ? (
					<>
						...
						{!auth?.app?.hasOnboarded && <Stack.Screen name={ONBOARD_SCREEN} component={Onboarding} />}

						{/* Only show screen if user has not accepted terms yet */}
						{!auth?.app?.termsAccepted && <Stack.Screen name={TERMS_AND_POLICY_SCREEN} component={TermsAndPrivacy} />}
